...
function upgrade_module_2_8_0($module)
{
    $add_table = Db::getInstance()->execute('
        CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'nts_config_payment_method` (
    ...
    if (!$add_table) {
        PrestaShopLogger::addLog('Could not add payment method config table. ' . Db::getInstance()->getMsgError(), 3);
