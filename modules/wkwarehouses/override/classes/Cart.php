...
{
    /*
    * Ensure including taxes within shipping cost for each package
	* Attention: Do not put each var of this function on its own line.
    */
/*
    public function getPackageShippingCost($id_carrier = null, $use_tax = true, Country $default_country = null, $product_list = null, $id_zone = null, bool $keepOrderPrices = false)
	{
        ...
        if (!empty($id_carrier)) {
            ...
            if (!empty($address_id)) {
                ...
                // Get all available taxes for the selected country
                $taxes = TaxRulesGroup::getAssociatedTaxRatesByIdCountry($country->id);
                // If equal, no problem, otherwise add taxe price
...
*/
    ...
    /*
    * Check quantities if advanced stock management ?
    */
    public function checkQuantities($returnProductOnFailure = false)
    {
        ...
        // To have for each package its shipping cost (with its own tax), reset the main address delivery ID from ps_cart
        ...
        foreach ($this->getProducts() as $product) {
            ...
            if (!$this->allow_seperated_package && !$product['allow_oosp'] &&
                ...
                /* IF Product handled by A.S.M but not available yet in any warehouse */
    ...
    public function getCarriersIntersection($packages_carriers, $id_package)
    {
        ...
        //ksort($packages_carriers); // sort back asc
        $target = $id_package - 1;
        if ($target == -1) {
            return null;
        }
    ...
    /*
    * Reduce the deep of each main package to minimize shipping cost, number of packages, etc.
    */
    public function getPackageList($flush = false)
    {
		// wk warehouses management module
        if (Module::isEnabled('wkwarehouses') && Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
        	...
        	// IF no A.S.M products in cart, return back to parent function
			if (WarehouseStock::getNumberOfAsmProductsInCart($this->id) > 0) {
				$package_list = $this->getMyPackageList($flush);
				/*
				 * Get unique package list if products in cart from different warehouses but with common carrier
				*/
				...
				// Look for carrier intersection for each group of packages
				// if we have in cart products (stored in warehouse) AND normal products (Not handled by A.S.M)
				if ($multi_packages) {
					...
					foreach ($package_list as $id_address => $packages) {
						if (count($packages) > 1) {
                    		...
                    		// Loop through warehouses arrays
							foreach ($packages_warehouses as $array_warehouse) {
								$ids_warehouses[] = (int)key($array_warehouse);
							}
                    		// IF different warehouses, no need to look for carrier intersection
    ...
    /**
     * Get all deliveries options combinations available for the current cart.
	 * This is an override to extended the multi-carriers choices
	 */
    ...
    /*
    * Just after the address creation (during checkout process)
    */
    ...
    /*
    * Assign warehouses to duplicated products in cart
    */
    public function duplicate()
    {
        ...
        if ($duplication && Validate::isLoadedObject($duplication['cart']) && $duplication['success']) {
			...
			// Don't accept products not associated yet to any warehouse
            ...
            * select only the concerned products of the order to duplicate (don't take also brothers orders)
            */
            if ($action == 'duplicateOrder' && Tools::getValue('tab') == 'AdminCarts') {
                /* get the right products of the order to duplicate */
            ...
            // Check delivery addresses in cart
            ...
            // IF We're coming from FrontOffice
