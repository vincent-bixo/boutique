                    ...
                    {if $comment.content}
                        <div class="comment clearfix" itemprop="review" itemscope itemtype="https://schema.org/Review">
                            <div class="comment_author" >
                                <span>{l s='Grade' d='Shop.Theme.Global'}&nbsp</span>
                                ...
                                <ul>
                                    ...
                                    {if $logged}
                                        {if !$comment.customer_advice}
                                            <li>{l s='Was this comment useful to you?' d='Shop.Theme.Global'}
                                                <button class="usefulness_btn" data-is-usefull="1" data-id-product-comment="{$comment.id_product_comment}">{l s='yes' d='Shop.Theme.Global'}</button>
                                                <button class="usefulness_btn" data-is-usefull="0" data-id-product-comment="{$comment.id_product_comment}">{l s='no' d='Shop.Theme.Global'}</button>
                                            </li>
                                        {/if}
                                        {if !$comment.customer_report}
                                            <li><span class="report_btn" data-id-product-comment="{$comment.id_product_comment}">{l s='Report abuse' d='Shop.Theme.Global'}</span></li>
                                        {/if}
                                    {/if}
                                </ul>
            ...
            {else}
                {if (!$too_early AND ($logged OR $allow_guests))}
                    <p class="align_center alert alert-info">
                        <a id="new_comment_tab_btn" class="open-comment-form" href="#new_comment_form">{l s='Be the first to write your review' d='Shop.Theme.Global'} !</a>
                    </p>
                {else}
                    <p class="align_center">{l s='No customer reviews for the moment.' d='Shop.Theme.Global'}</p>
                {/if}
            {/if}
            <div class="clearfix pull-left">
                {if ($too_early == false AND ($logged OR $allow_guests))}
                    <a class="open-comment-form btn btn-primary" href="#new_comment_form">{l s='Write your review' d='Shop.Theme.Global'}</a>
                {/if}
            </div>
        ...
        <div style="display:none">
            <div id="new_comment_form">
                <form id="id_new_comment_form" action="#">
                    <h2 class="title">{l s='Write your review' d='Shop.Theme.Global'}</h2>
                    {if isset($productcomments_product) && $productcomments_product}
                    ...
                    <div class="new_comment_form_content">
                        <h2>{l s='Write your review' d='Shop.Theme.Global'}</h2>
                        ...
                        {/if}
                        <label for="comment_title">{l s='Title for your review' d='Shop.Theme.Global'}<sup class="required">*</sup></label>
                        <input id="comment_title" name="title" type="text" value=""/>

                        <label for="content">{l s='Your review' d='Shop.Theme.Global'}<sup class="required">*</sup></label>
                        <textarea id="content" name="content"></textarea>

                        {if $allow_guests == true && !$logged}
                            <label>{l s='Your name' d='Shop.Theme.Global'}<sup class="required">*</sup></label>
                            <input id="commentCustomerName" name="customer_name" type="text" value=""/>
                        {/if}

                        <div id="new_comment_form_footer">
                            <input id="id_product_comment_send" name="id_product" type="hidden" value='{$id_product_comment_form}'/>
                            <p class="row required"><sup>*</sup> {l s='Required fields' d='Shop.Theme.Global'}</p>
                            <p class="fr">
                                {if $PRODUCT_COMMENTS_GDPR == 1}
                                {literal}
                                    <input onchange="if($(this).is(':checked')){$('#submitNewMessage').removeClass('gdpr_disabled'); $('#submitNewMessage').removeAttr('disabled'); rebindClickButton();}else{$('#submitNewMessage').addClass('gdpr_disabled'); $('#submitNewMessage').off('click'); $('#submitNewMessage').attr('disabled', 1); }" id="gdpr_checkbox" type="checkbox" >
                                {/literal}
                                    {l s='I accept ' d='Shop.Theme.Global'} <a target="_blank" href="{$link->getCmsLink($PRODUCT_COMMENTS_GDPRCMS)}">{l s='privacy policy' d='Shop.Theme.Global'}</a> {l s='rules' d='Shop.Theme.Global'}
                                {/if}

                                <button {if $PRODUCT_COMMENTS_GDPR == 1}disabled{/if} class="btn btn-primary {if $PRODUCT_COMMENTS_GDPR == 1}gdpr_disabled{/if}" id="submitNewMessage" name="submitMessage" type="submit">{l s='Send' d='Shop.Theme.Global'}</button>&nbsp;
                                {l s='or' d='Shop.Theme.Global'}&nbsp;<a href="#" onclick="$.fancybox.close();">{l s='Cancel' d='Shop.Theme.Global'}</a>
                            </p>
                            <div class="clearfix"></div>
                        </div>
                    </div>
