...
class Tm_Categorylist extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('PX - Category List Block', array(), 'Modules.CategoryList');
        $this->description = $this->trans('Adds a block with sub category.', array(), 'Modules.CategoryList');
		
        $this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
		$this->templateFile = 'module:tm_categorylist/views/templates/front/tm_categorylist.tpl';
    }
	
	public function install()
    {	
		$arrayDefault = array('3','4','5','6');
        Configuration::updateValue("TM_CATE_LIST", serialize($arrayDefault));
	...
	public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmcategorylist` (
			`id_tmcategorylist` int(11) NOT NULL AUTO_INCREMENT,
			`image` varchar(128) NOT NULL,
			`id_shop` int(10) NOT NULL,
			`name_category` varchar(128) NOT NULL,
			`id_category` int(10)  NOT NULL,		
			PRIMARY KEY (`id_tmcategorylist`)
    ...
    public function uninstallDB($drop_table = true)
    {
		...
		$ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmcategorylist`');
        }
    ...
    {
        $output = '';
        if (((bool)Tools::isSubmit('submitTmcategorylist')) == true) {
        ...
        if (Tools::isSubmit('deletetm_categorylist') && Tools::getValue('id_category')) {
    ...
    protected function renderForm()
    {
        ...
        $helper->submit_action = 'submitTmcategorylist';
    ...
    {
        $result = Db::getInstance()->execute('INSERT  INTO `'._DB_PREFIX_.'tmcategorylist`(`id_category`,`name_category`,`image`,`id_shop`) VALUES ('.(int)$category_id.', \''.pSQL($name_cate).'\', \''.pSQL($filename).'\','.(int)$id_shop.')');
        return $result;
    }

    public function updatecategoryicon($category_id,$name_cate,$filename)
    {
        $result = Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'tmcategorylist` SET `name_category` = \''.pSQL($name_cate).'\',`image` =\''.pSQL($filename).'\' WHERE `id_category` = '.(int)$category_id);
        return $result;
    }

    public static function deleteCategoryId($category_id)
    {
        $sql = 'DELETE FROM`'._DB_PREFIX_.'tmcategorylist` WHERE `id_category` = '.(int)$category_id;
        Db::getInstance()->execute($sql);
    }
    
    public function getimage($category_id,$id_shop)
    {
        $sql = 'SELECT id_tmcategorylist ,image,id_category,name_category FROM '._DB_PREFIX_.'tmcategorylist WHERE id_category = '.(int)$category_id.' and id_shop ='.(int)$id_shop.'' ;
        return Db::getInstance()->executeS($sql);
    }

    public function getcategoryicon()
    {
        $sql = 'SELECT id_tmcategorylist ,image,id_category,name_category, id_shop FROM '._DB_PREFIX_.'tmcategorylist';
        return Db::getInstance()->executeS($sql);
    }
	
	public function hookdisplayHeader($params)
    {
        $this->context->controller->registerStylesheet('modules-tmcategorylist', 'modules/'.$this->name.'/views/css/tmcategorylist.css', ['media' => 'all', 'priority' => 150]);
    }
	
    public function renderWidget($hookName = null, array $configuration = [])
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('tm_categorylist'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('tm_categorylist'));
    }
	 ...
	 public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        return array(
			 ...
			 'image_url' => $this->context->link->getMediaLink(_MODULE_DIR_.'tm_categorylist/views/img'),
        );
    }
}
