...
class Tm_Imagehover extends Module
{	
	public function __construct()
	{
		$this->name = 'tm_imagehover';
		...
		$this->displayName = $this->trans('PX - Product Hover Image', array(), 'Modules.ImageHover');
	...
	{
		return parent::install() && $this->registerHook('displayTmHoverImage');
	}
	
	public function hookDisplayTmHoverImage($params) {
		
		if (!$this->isCached('tm_imagehover.tpl', $this->getCacheId($params['id_product']))) {
		...
		return $this->display(__FILE__, 'tm_imagehover.tpl', $this->getCacheId($params['id_product']));
	}
}